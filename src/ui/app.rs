...
use crate::ui::render::{draw_board, board_positions};
...
pub async fn run_ui() {
    ...
    loop {
        ...
        if let Some(id) = losange_clicked(board_x, board_y, cell_size) && selected_losange == None {
        ...
        for losange in losanges{
            if selected_losange == Some(losange.id){
                draw_rectangle(losange.screen_x - 40.0, losange.screen_y - 40.0, cell_size - 20.0, cell_size - 20.0, Color::from_rgba(0, 255, 0, 128));
            }
        }
